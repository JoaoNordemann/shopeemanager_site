...
function configurarGateDeAcesso() {
    const form = document.querySelector("#gate-form");
    const campoUsuario = document.querySelector("#gate-user");
    const campoSenha = document.querySelector("#gate-pass");
    const mensagemErro = document.querySelector("#gate-error");
    if (!form || !campoUsuario || !campoSenha || !mensagemErro)
        return;
...
function configurarBotoesDeDownload() {
    const botoes = document.querySelectorAll("[data-download-btn]");
...
function revelarCartoesAoRolar() {
    const cartoes = document.querySelectorAll(".feature-card");
    if (cartoes.length === 0)
        return;
